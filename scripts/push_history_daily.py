...
files=['app/page.tsx','app/history/page.tsx']
...
commit=json.loads(subprocess.run([GH,'api',f'repos/{REPO}/git/commits','--input','-'],input=json.dumps({'message':'feat: improve history records and daily match auto display','tree':new_tree['sha'],'parents':[parent]}),capture_output=True,text=True,timeout=30).stdout)
