...
files=['lib/mockData.ts','app/page.tsx','app/matches/page.tsx','app/history/page.tsx']
...
commit=json.loads(subprocess.run([GH,'api',f'repos/{REPO}/git/commits','--input','-'],input=json.dumps({'message':'feat: update group-stage schedule from provided fixtures','tree':new_tree['sha'],'parents':[parent]}),capture_output=True,text=True,timeout=30).stdout)
