...
files = ['components/ProModal.tsx', 'next.config.js']
...
commit = json.loads(subprocess.run([GH,'api',f'repos/{REPO}/git/commits','--input','-'],input=json.dumps({'message':'fix: QR image paths with basePath','tree':new_tree['sha'],'parents':[parent]}),capture_output=True,text=True,timeout=30).stdout)
