...
files_to_push = [
    'lib/mockData.ts',  # history records fix
    'lib/auth.tsx',  # user stats functions
    'app/globals.css',  # blur-mask stronger
    'components/ScoreProbability.tsx',  # bg-white/95
    'app/admin/page.tsx',  # stats cards
]
...
commit = json.loads(subprocess.run([GH,'api',f'repos/{REPO}/git/commits','--input','-'],
    input=json.dumps({'message':'fix: history dates + pro blur + admin stats','tree':new_tree['sha'],'parents':[parent]}),
    capture_output=True, text=True, timeout=30).stdout)
