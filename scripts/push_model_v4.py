...
files=['lib/mockData.ts','app/matches/[id]/page.tsx']
...
commit=json.loads(subprocess.run([GH,'api',f'repos/{REPO}/git/commits','--input','-'],input=json.dumps({'message':'feat: upgrade prediction model with risk and style adjustments','tree':new_tree['sha'],'parents':[parent]}),capture_output=True,text=True,timeout=30).stdout)
